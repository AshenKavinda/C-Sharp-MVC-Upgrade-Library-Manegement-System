...
namespace Library
{
    internal class Payment
    {
        ...
        public string addRecordPayment(String CID,int price,String type)
        {
            try
            {
                double nextPID = PIDnumaricValue + 1;
                string PID = "P" + nextPID.ToString("000000");
                conn.Open();
        ...
        public string addRegisterPayment(String CID)
        {
            return addRecordPayment(CID, settings.getRegisterFee(), "R");
        }

        public string addMonthlyPayment(String Data,int value)
        {
            String CID = customer.getCIDByCIDorPno(Data);
            return addRecordPayment(CID, value,"M");
        }

        public int setMonthlyPayment(String Data,int month,string PID)
        {
        ...
        { 
            ...
            {
                String CID = customer.getCIDByCIDorPno(Data);
                string PID = addRecordPayment(CID, value, "O");
                if (PID != null)
                {
                    conn.Open();
