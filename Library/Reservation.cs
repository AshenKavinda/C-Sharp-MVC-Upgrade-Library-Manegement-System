...
namespace Library
{
    internal class Reservation
    {
        ...
        {      
            ...
            {
                conn.Open();
                SqlCommand sqlCommand = new SqlCommand("Select * from Reservation where CID = '" + CID + "' and Status = '"+1+"'", conn);
                ...
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        if (differenceInDays > settings.getOverDueLimit())
                        {
        ...
        public int[] checkEligibilityOfCustomerByCIDandBID(String CID,String BID)
        {
            ...
            int overDueBookLimit = settings.getOverDueBookLimit();
            if (table.Rows.Count >= overDueBookLimit) //overDue Book Limit
            {
                eligibility[2] = -1;
            }
        ...
        public int addReservation(String CID,String BID)
        {
            ...
            try
            {
                double nextRID = RIDNumaricValue + 1 ;
                String RID = "R" + nextRID.ToString("000000");
